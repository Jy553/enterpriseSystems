...
class SmartMeterApp(ctk.CTk):
            ...
            for iface_name, iface_list in mac.items():
                for iface in iface_list:
                    if iface.family == psutil.AF_LINK:
                        return hashlib.sha256(iface.address.encode('utf-8')).hexdigest()[:16]
    ...
    def start_receiving_messages(self):
        ...
        try:
            bills = Thread(target=self.receive_billing_messages, args=[self.APIHandler.declare_reply_queue()])
            bills.daemon = True
            bills.start()
    ...
    def update_bill(self):
        try:
            with self.APIHandler as handler:
                while True:
            ...
            self.log_to_console(f"Failed to send message due to invalid data : {e}")
        except Exception as e:
